...
def process_single_script(script_name, folder_path):
    ...
    try:
        ...
        if not data.empty:
            if script_name.endswith(".csv"):
                csv_name = script_name
            elif script_name.endswith(".py"):
                csv_name = script_name.replace('.py', '.csv')
            elif ".csv" not in script_path or ".py" not in script_path:
                csv_name = f"{script_name}.csv"
            else:
                csv_name = script_name.replace('.py', '.csv')
            ...
            logging.warning(f"No data returned for script: {script_name}.")
...
def process_script(script_name, folder_path, summary_stats):
    try:
        ...
        # If an error occurs, don't increment the stats for this script
